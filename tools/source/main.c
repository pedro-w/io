...
double System_UserTime(void) {
    ...
    return r == -1
               ? -1
               : u.ru_utime.tv_sec + (((double)u.ru_utime.tv_usec) / 1000000);
}
...
int main(int argc, const char *argv[]) {
    ...
    self = IoState_new();
#ifdef IOBINDINGS
    IoState_setBindingsInitCallback(
...
#ifdef IO_SHOW_STATS
    {
        ...
        printf("[ %.3fs user  %.3fs total  %.1f%% gc  %i sweeps  %i frees  "
               "%.3fmb max ]\n",
               System_UserTime(), totalTime, 100.0 * collectorTime / totalTime,
               (int)sweepCount, (int)io_frees(), maxAllocatedBytes / 1048576.0);
