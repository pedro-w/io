...
const char *IoEvalFrame_stateName(IoFrameState state) {
    ...
    case FRAME_STATE_IF_CONVERT_BOOLEAN:
        return "if:convertBoolean";
    case FRAME_STATE_IF_EVAL_BRANCH:
        return "if:evalBranch";
    case FRAME_STATE_WHILE_EVAL_CONDITION:
        return "while:evalCondition";
    case FRAME_STATE_WHILE_CHECK_CONDITION:
        return "while:checkCondition";
    case FRAME_STATE_WHILE_DECIDE:
        return "while:decide";
    case FRAME_STATE_WHILE_EVAL_BODY:
        return "while:evalBody";
    case FRAME_STATE_LOOP_EVAL_BODY:
        return "loop:evalBody";
    case FRAME_STATE_LOOP_AFTER_BODY:
        return "loop:afterBody";
    case FRAME_STATE_FOR_EVAL_SETUP:
        return "for:evalSetup";
    case FRAME_STATE_FOR_EVAL_BODY:
        return "for:evalBody";
    case FRAME_STATE_FOR_AFTER_BODY:
        return "for:afterBody";
    case FRAME_STATE_FOREACH_EVAL_BODY:
        return "foreach:evalBody";
    case FRAME_STATE_FOREACH_AFTER_BODY:
        return "foreach:afterBody";
#ifdef IO_CALLCC
    case FRAME_STATE_CALLCC_EVAL_BLOCK:
        return "callcc:evalBlock";
#endif
    case FRAME_STATE_CORO_WAIT_CHILD:
        return "coro:waitChild";
    case FRAME_STATE_CORO_YIELDED:
        return "coro:yielded";
    case FRAME_STATE_DO_EVAL:
        return "do:eval";
...
// Return a frame state enum from its string name (reverse of
...
IoFrameState IoEvalFrame_stateFromName(const char *name) {
    ...
    if (strcmp(name, "start") == 0)
        return FRAME_STATE_START;
    if (strcmp(name, "evalArgs") == 0)
        return FRAME_STATE_EVAL_ARGS;
    if (strcmp(name, "lookupSlot") == 0)
        return FRAME_STATE_LOOKUP_SLOT;
    if (strcmp(name, "activate") == 0)
        return FRAME_STATE_ACTIVATE;
    if (strcmp(name, "continueChain") == 0)
        return FRAME_STATE_CONTINUE_CHAIN;
    if (strcmp(name, "return") == 0)
        return FRAME_STATE_RETURN;
    if (strcmp(name, "if:evalCondition") == 0)
        return FRAME_STATE_IF_EVAL_CONDITION;
    if (strcmp(name, "if:convertBoolean") == 0)
        return FRAME_STATE_IF_CONVERT_BOOLEAN;
    if (strcmp(name, "if:evalBranch") == 0)
        return FRAME_STATE_IF_EVAL_BRANCH;
    if (strcmp(name, "while:evalCondition") == 0)
        return FRAME_STATE_WHILE_EVAL_CONDITION;
    if (strcmp(name, "while:checkCondition") == 0)
        return FRAME_STATE_WHILE_CHECK_CONDITION;
    if (strcmp(name, "while:decide") == 0)
        return FRAME_STATE_WHILE_DECIDE;
    if (strcmp(name, "while:evalBody") == 0)
        return FRAME_STATE_WHILE_EVAL_BODY;
    if (strcmp(name, "loop:evalBody") == 0)
        return FRAME_STATE_LOOP_EVAL_BODY;
    if (strcmp(name, "loop:afterBody") == 0)
        return FRAME_STATE_LOOP_AFTER_BODY;
    if (strcmp(name, "for:evalSetup") == 0)
        return FRAME_STATE_FOR_EVAL_SETUP;
    if (strcmp(name, "for:evalBody") == 0)
        return FRAME_STATE_FOR_EVAL_BODY;
    if (strcmp(name, "for:afterBody") == 0)
        return FRAME_STATE_FOR_AFTER_BODY;
    if (strcmp(name, "foreach:evalBody") == 0)
        return FRAME_STATE_FOREACH_EVAL_BODY;
    if (strcmp(name, "foreach:afterBody") == 0)
        return FRAME_STATE_FOREACH_AFTER_BODY;
#ifdef IO_CALLCC
    if (strcmp(name, "callcc:evalBlock") == 0)
        return FRAME_STATE_CALLCC_EVAL_BLOCK;
#endif
    if (strcmp(name, "coro:waitChild") == 0)
        return FRAME_STATE_CORO_WAIT_CHILD;
    if (strcmp(name, "coro:yielded") == 0)
        return FRAME_STATE_CORO_YIELDED;
    if (strcmp(name, "do:eval") == 0)
        return FRAME_STATE_DO_EVAL;
    if (strcmp(name, "do:wait") == 0)
        return FRAME_STATE_DO_WAIT;
    return FRAME_STATE_START;
}
...
IO_METHOD(IoEvalFrame, description) {
    ...
    */
    ...
    if (fd->message) {
        ...
        char buf[512];
        snprintf(buf, sizeof(buf), "%s %s:%d [%s]", msgName, label, line,
                 stateName);
        return IOSYMBOL(buf);
    }
