...
typedef struct IoEvalFrameData {
    ...
    IoCall
        *savedCall; // Original Call preserved across in-place if optimization
    ...
    uintptr_t
        retainPoolMark; // Retain pool mark for block activation (0 if none)
    ...
    union {
        ...
        struct {
            ...
            IoObject
                *mapSource; // Original Map (when collection is a keys list)
        } foreachInfo;
...
#define CACHED_LITERAL_RESULT(bodyMsg) (IOMESSAGEDATA(bodyMsg)->cachedResult)
